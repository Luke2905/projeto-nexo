...
class OAuthService {
  constructor(private client: ReturnType<typeof axios.create>) {
    ...
    if (!ENV.oAuthServerUrl) {
      console.warn(
        "[OAuth] OAUTH_SERVER_URL is not configured. Local auth will be used."
      );
    }
  }
...
class SDKServer {
  ...
  async verifySession(
    ...
    try {
      ...
      if (!isNonEmptyString(openId) || typeof appId !== "string") {
  ...
  async authenticateRequest(req: Request): Promise<AuthenticatedUser> {
    ...
    if (!user && sessionUserId.startsWith("local_")) throw ForbiddenError("Session revoked");
    ...
    await db.touchUser(user.id, signedInAt);
